...
def generate_score(style, tempo, key):
    ...
    Args:
        style (str): 생성할 악보의 음악 스타일 (예: "classical", "jazz", "rock")
        tempo (str): 생성할 악보의 템포 (예: "allegro", "andante", "largo")
        key (str): 생성할 악보의 조성 (예: "C major", "G minor")

    Returns:
        None: 생성된 악보 이미지를 파일로 저장.
    """
...
def encode_style(style:str) -> torch.Tensor:
    ...
    Args:
        style (str): 인코딩할 음악 스타일

    Returns:
        torch.Tensor: 인코딩된 스타일 벡터
    """
...
def encode_tempo(tempo):
    ...
    Args:
        tempo (str): 인코딩할 템포 (예: "slow", "moderate", "fast")

    Returns:
        torch.Tensor: 인코딩된 템포 벡터
    """
...
def encode_key(key):
    ...
    Args:
        key (str): 인코딩할 조성 (예: "C major", "A minor")

    Returns:
        torch.Tensor: 인코딩된 조성 벡터
    """
...
def transform_to_image(tensor):
    ...
    Args:
        tensor (torch.Tensor): 변환할 텐서. Shape: (1, 1, height, width)

    Returns:
        PIL.Image.Image: 변환된 PIL 이미지 객체
    """
